    ...
    fn call(&self, mut req: ServiceRequest) -> Self::Future {
      ...
      Box::pin(async move{
        let app_state = match req.app_data::<Data<AppState>>() {
            Some(d) => d.clone(),
            None => return Err(actix_web::error::ErrorInternalServerError("app state error"))
        }; 
        let token_option = req.headers().get(AUTHORIZATION).and_then(|h| h.to_str().ok()).and_then(|s| {
